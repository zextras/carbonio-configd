	...
	go func() {
		...
		for {
			...
			case syscall.SIGINT, syscall.SIGTERM:
				...
				// Notify systemd that we are stopping before cancelling the context
				if err := notifier.Stopping(); err != nil {
					logger.ErrorContext(ctx, "Failed to send sd_notify STOPPING",
						"error", err)
				}

				cancel() // Cancel context immediately to interrupt all operations

				return // Exit the goroutine
			case syscall.SIGHUP, syscall.SIGUSR2, syscall.SIGALRM:
				// These signals should trigger a recheck/reload of configurations
				// and wake up the main loop if it's sleeping.
				appState.SetSleepTimer(0) // Reset sleep timer to wake up main loop

				// Notify systemd that we are reloading configuration
				...
				logger.InfoContext(ctx, "Triggering configuration reload",
					"signal", sig.String())
				// Attempt to send reload signal, non-blocking
				select {
			...
			case syscall.SIGCHLD:
				// SIGCHLD (child process exit) is logged but does NOT trigger reload.
				// This prevents excessive reload cycles from zmprov/postconf command exits.
				// Matches Python/Jython zmconfigd behavior where SIGCHLD is effectively ignored.
