...
func (g *Generator) upstreamCandidates(ctx context.Context, spec *upstreamSpec) ([]upstreamCandidate, error) {
	...
	if spec.attrList != "" {
		names := g.attrListServerNames(spec.attrList)
		selected = selectByAttrList(byHost, names)
	} else {
		selected = selectByServices(byHost, spec.services)
	}
...
// selectByAttrList returns servers whose hostname appears in names.
func selectByAttrList(byHost map[string]map[string]string, names []string) map[string]map[string]string {
...
// selectByServices returns servers that enable at least one of the given services.
func selectByServices(byHost map[string]map[string]string, services []string) map[string]map[string]string {
	selected := make(map[string]map[string]string)

	for _, attrs := range byHost {
		if serverEnablesAnyService(attrs, services) {
